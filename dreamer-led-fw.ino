...
void drw(){
  ...
  for ( int y = 0; y < LED_MATRIX_HEIGHT; y++ ){
    for ( int x = 0; x < LED_MATRIX_WIDTH; x++ ){
      idx = (x + y * LED_MATRIX_WIDTH) * 3;
      if ( 720 < bitmap[idx] + bitmap[idx+1]  + bitmap[idx+2] ){
        leds[ XY(x, y)] = CRGB::Black;
      } else {
        leds[ XY(x, y)].r = bitmap[idx];
        leds[ XY(x, y)].g = bitmap[idx+1];
        leds[ XY(x, y)].b = bitmap[idx+2];
      }
    }    
  }
}
...
void loop() {
  EVERY_N_SECONDS(1) {
    ...
    leds[ XY(LED_MATRIX_WIDTH-1, LED_MATRIX_HEIGHT-1)]  = CRGB::Yellow;
    
    leds[ XY(LED_MATRIX_WIDTH-1, 0)]  = CRGB::Green;
    leds[ XY(0, LED_MATRIX_HEIGHT-1)]  = CRGB::Blue;    
