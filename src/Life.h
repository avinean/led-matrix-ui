...
#include "g_snake.h_"
#include "../.pio/libdeps/esp32dev/FastLED@src-42531654dd09e157112a9562ad6d0e37/pixeltypes.h"
...
public:
    ...
    void newGame() {
        matrix->clear();
        effects.leds[XY_EF( 0, 1 )] = (CRGB)GLOBAL_COLOR_1;
        effects.leds[XY_EF( 1, 1 )] = (CRGB)GLOBAL_COLOR_1;
        effects.leds[XY_EF( 2, 1 )] = (CRGB)GLOBAL_COLOR_1;
        matrix->show();
    }
...
//                    }
            ...
            for ( uint8_t y = 0; y < _h; y++ )
                for (uint8_t x = 0; x < _w; x++)
                    if ( map[x][y] > 0 )
                        effects.leds[XY_EF(x, y)] = (CRGB)GLOBAL_COLOR_1;
                    else
                        effects.leds[XY_EF(x, y)] = (CRGB)CRGB::Black;
