...
//    matrix->drawPixel(headX - i, headY, (uint16_t)GLOBAL_COLOR_1);
            effects.leds[XY_EF(headX - i, headY)] = GLOBAL_COLOR_1;
            buttVector[i] = 0;
        }
        FastLED.show();
...
//        effects.leds[xy] = GLOBAL_COLOR_2;
                ...
                if (getPixColorXYCRGB(appleX, appleY) == (CRGB)CRGB::Black) {
//        if (_col == CRGB::Black) {
...
//        matrix->drawPixel(appleX, appleY, (uint16_t)GLOBAL_COLOR_2 );        // и рисуем
                    effects.leds[XY_EF(appleX, appleY)] = GLOBAL_COLOR_2;
                    FastLED.show();
                }
            }
...
//        matrix->drawPixel(buttX, buttY, (uint16_t)0x000000);
                    effects.leds[XY_EF(buttX, buttY)] = CRGB::Black;
                }
